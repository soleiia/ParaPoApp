...
class _MapPageState extends State<MapPage> {
  ...
  void _onStateChange() {
    if (AppState.instance.hasPendingRoute) {
      _checkPendingRoute();
    }
  }
  ...
  Future<void> _startNavigation() async {
    ...
    // Try to find matching route in DB to get coordinates for geocoding
    AppLatLng? origin = _originPin;
    AppLatLng? dest   = _destPin;

    // If no pins preset, search routes for a coordinate hint
    // (OSRM can also geocode from the name if no coords found)
    if (origin == null || dest == null) {
      final routes = await _routeRepo.search(from);
      final match  = routes.where((r) =>
          r.destination.toLowerCase().contains(to.toLowerCase()))
          .firstOrNull;
      // If exact match found in DB, use hardcoded Cabuyao center as fallback
      // Real app: integrate a geocoding API here
      if (match != null) {
        // Use approximate Cabuyao center coordinates as starting point
        // OSRM will route on the actual road network from these points
        origin = const AppLatLng(14.2724, 121.1241);
        dest   = const AppLatLng(14.2100, 121.1639); // approximate Calamba
    ...
    if (result == null) {
      setState(() {
        _loading  = false;
        _errorMsg =
            'Could not get directions. Check your internet connection.';
      });
      return;
    }
    ...
    return Stack(children: [
      Column(children: [
        ...
        Container(color: AppColors.blue,
          child: SafeArea(bottom: false,
            child: Padding(
              ...
              child: Row(children: [
                const Text('Cabuyao', style: TextStyle(color: Colors.white,
        ...
        Expanded(child: Stack(children: [
          ...
          Positioned(top: 16, left: 16, right: 16,
            child: Container(
              ...
              child: Column(children: [
                _pointRow(_fromCtrl, 'INPUT POINT A (e.g. Cabuyao City Hall)',
                    AppColors.blue, null,
          ...
          // My location button
          Positioned(bottom: _showSheet ? 300 : 80, right: 16,
            child: _FloatBtn(icon: Icons.my_location,
            ...
            Positioned(bottom: 24, right: 16,
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: _loading
                      ? const SizedBox(width: 88, child: Center(
                          child: SizedBox(width: 18, height: 18,
                            child: CircularProgressIndicator(
                                color: Colors.white, strokeWidth: 2))))
                      : const Text('START NAVIGATION', style: TextStyle(
...
class _RouteSheet extends StatefulWidget {
  ...
  const _RouteSheet({
    required this.directions, required this.fromText, required this.toText,
    required this.onClose, required this.onShare, required this.onSave,
  });
  @override State<_RouteSheet> createState() => _RouteSheetState();
}
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(d.totalDuration, style: const TextStyle(color: Colors.white,
                    fontWeight: FontWeight.w800, fontSize: 22)),
            ...
            Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                Text(widget.fromText, style: const TextStyle(
                    color: Colors.white, fontWeight: FontWeight.w600,
                    fontSize: 13),
                    overflow: TextOverflow.ellipsis),
                Row(children: const [
                  Icon(Icons.arrow_downward, color: Colors.white54, size: 12),
                  SizedBox(width: 4),
                ]),
                Text(widget.toText, style: const TextStyle(
                    color: Colors.white70, fontSize: 13),
                    overflow: TextOverflow.ellipsis),
              ])),
              const Spacer(),
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                ListenableBuilder(listenable: AppState.instance,
                  builder: (_, __) {
                    const base = 14.0; // min jeepney fare
                    final fare = AppState.instance.isDiscounted
                        ? base * 0.80 : base;
                    return Text('₱${fare.toStringAsFixed(2)}',
            ...
            if (d.steps.isNotEmpty) ...[
              ...
              GestureDetector(
                ...
                child: Row(children: [
                  const Icon(Icons.directions, color: Colors.white70, size: 16),
            ...
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: YellowBtn(label: 'Confirm', icon: Icons.check,
