...
class TransportationPage extends StatefulWidget {
  ...
  @override State<TransportationPage> createState() => _TransportationPageState();
}
...
class _TransportationPageState extends State<TransportationPage> {
  ...
  void _showAddDialog() {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, sd) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        buildDialogField(nameCtrl, 'Transport Name'),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(padding: const EdgeInsets.all(16),
            ...
            child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                const Text('Discounted Fare (20% off)', style: TextStyle(color: AppColors.textMid, fontSize: 14)),
                Text('Ph${item.fareFor(true).toStringAsFixed(2)}',
                    style: const TextStyle(color: Color(0xFF2E7D32), fontSize: 14, fontWeight: FontWeight.w700)),
              ]),
  ...
  void _showEditDialog(TransportModel item) {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, sd) => AlertDialog(
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel', style: TextStyle(color: AppColors.textMid))),
        TextButton(onPressed: () async {
          await _repo.update(item.copyWith(name: nameCtrl.text.trim(),
              fare: double.tryParse(fareCtrl.text) ?? item.fare, active: active, emoji: emoji));
          if (ctx.mounted) { Navigator.pop(ctx); await _load(); }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      PageHeader(title: 'Transportation Options', onAdd: _showAddDialog, onSearch: _onSearch,
